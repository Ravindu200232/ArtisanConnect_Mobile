  ...
  const handleDeleteRestaurant = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await axios.delete(`http://localhost:3000/api/v1/restaurant/delete/${id}`, {
  ...
  const handleToggleShop = async (id, shouldOpen) => {
    try {
      ...
      const endpoint = shouldOpen
        ? `http://localhost:3000/api/v1/restaurant/isOpen/${id}`
        : `http://localhost:3000/api/v1/restaurant/isClose/${id}`;
