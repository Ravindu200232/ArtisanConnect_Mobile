  ...
  const handleDeleteShop = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await axios.delete(`http://localhost:3000/api/v1/owner/delete/${id}`, {
  ...
  const handleToggleShop = async (id, shouldOpen) => {
    try {
      ...
      const endpoint = shouldOpen
        ? `http://localhost:3000/api/v1/owner/isOpen/${id}`
        : `http://localhost:3000/api/v1/owner/isClose/${id}`;
